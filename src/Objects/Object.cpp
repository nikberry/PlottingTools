...
TH1D* Object::readGe2bHistogram(Sample sample, Variable variable) {
	...
	cout << "plot: " << "TTbar_plus_X_analysis/MuPlusJets/Ref selection/"+objName+"/"+variable.name << endl;

	TH1D* plot = (TH1D*) sample.file->Get("TTbar_plus_X_analysis/MuPlusJets/Ref selection/"+objName+"/"+variable.name+"_2btags");
	TH1D* plot2 = (TH1D*) sample.file->Get("TTbar_plus_X_analysis/MuPlusJets/Ref selection/"+objName+"/"+variable.name+"_3btags");
	TH1D* plot3 = (TH1D*) sample.file->Get("TTbar_plus_X_analysis/MuPlusJets/Ref selection/"+objName+"/"+variable.name+"_4orMoreBtags");
...
//	Will need to see if this works in other situations
	cout << "width: " <<  ratio->GetBinWidth(1) << endl;
	cout << "min: " <<  variable.minX+ratio->GetBinWidth(1)/2 << endl;
	cout << "max: " <<  fabs(variable.minX+ratio->GetBinWidth(1)/2) << endl;
...
//	}else if(variable.minX < 0. && ratio->GetBinWidth(1) > 0.1){
//		ratio->SetAxisRange(variable.minX, fabs(variable.minX));
	}else{
